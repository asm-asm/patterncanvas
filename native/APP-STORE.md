...
> 2026-09-17：最新の提出素材は [release-1.0/README.md](release-1.0/README.md)。日本限定・買い切り3,000円・「編みものノート」・iPhone/iPad対応。署名とTestFlightは設定済み、候補ビルド14.1。以下の初期構築手順には過去の記録も含みます。
...
現状は提出準備です。審査通過はAppleが判断します。ネイティブ実機の確認、署名、App Store Connectの登録内容、公開プライバシーURLの確認、最終スクリーンショットが済むまでは提出しないでください。
...
公開URL：`https://asm-asm.github.io/patterncanvas/app-store/privacy/`。
紹介・サポート・プライバシーページをmainで公開済みです。提出直前にもリンクが開くことを確認してください。
...
## 表裏切り替えの位置保持（2026-09-19）

拡大・スクロール中は表示中の範囲を基準に表裏を返し、反対側の別の目へ飛ばないよう横位置も反転します。段と倍率は維持し、小さな編み図は同じ領域内で反転します。切り替え文言の高さを統一し、段の色・線幅設定は編み図見出しの歯車（44px以上、読み上げラベル付き）に移動しました。表編み・裏編みの目記号を自動変換する機能ではなく、色の編み図を往復編みで読むための表示切り替えです。

## 目番号の始まりと上端表示

表裏を返す表示機能は削除しました。編み図の上下に同じ目番号を表示し、「目番号の始まり」で左から1／右から1を選びます。柄、編集座標、現在段、スクロール位置は変化しません。旧版の表裏設定は使用せず、新しい番号設定を端末に保存します。上端と下端の番号欄は編集対象外です。

## 描画の取り消し

「元に戻す」「やり直す」を編み図／両方表示時の画面下に固定し、文字付き44px以上のボタンに変更しました。通常の描画の取り消しでは表示位置・倍率を維持します。目数・段数が変わる操作のみ再配置します。履歴は従来どおりアプリ実行中のものです。

## 増し目記号

M1L・M1R・M1LP・M1RP・KFB・YOを、色とは別の同サイズ二次元配列 `symbols` に保存します。新形式 `patterncanvas-web-3` は旧形式1・2も読めます。記号入りの作品は旧版では開けないため、最新版で使用してください。

「増し目記号」ツールは略号をマスへ置くだけで、色や目数は変更しません。「目・段の追加／削除」で記号を選んで目を追加すると、開始段の追加マスだけに記号を付け、それ以降の目数も増やします。KFBを含め、このアプリでは増えた1目のマスを注記します。編み地は各マスの色を使った表目の参考表示で、増し目の傾き・穴・裏目の実物形状を再現するものではありません。

色ペン・塗りつぶしは記号を保ち、消しゴムと範囲削除は色と記号を消します。「記号だけ消す」では色を保ちます。コピー・移動・段や目の挿入削除・リピートにも対応します。柄を左右反転する編集ではM1L↔M1R、M1LP↔M1RPを交換します。目番号の方向変更では記号を変更しません。

略号の参照：[Craft Yarn Council](https://craftyarncouncil.com/standards/knitting-abbreviations)、[Purl Soho M1L/M1R](https://www.purlsoho.com/create/make-one-right-m1r-make-one-left-m1l/)。
