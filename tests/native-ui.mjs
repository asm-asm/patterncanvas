...
try{
  ...
  await page.locator('#chart-mirror').scrollIntoViewIfNeeded();
  const stateBeforeTurn=await page.evaluate(()=>({project:localStorage.getItem('patterncanvas-iphone-v1'),scrollY,work:document.getElementById('work').scrollTop}));
  const footer=()=>c.evaluate(canvas=>window.chartLabels.filter(l=>l.y===canvas.getBoundingClientRect().height-14&&l.x>=72&&l.x<=canvas.getBoundingClientRect().width).map(l=>l.label));
  const numbersBefore=await footer();assert(numbersBefore.length>2);
  const imageBeforeTurn=await c.evaluate(c=>c.toDataURL());
  await page.locator('#chart-mirror').click();
  assert.deepEqual(await footer(),[...numbersBefore].reverse(),'Same visible stitch numbers, reversed, after turning a zoomed and panned chart');
  assert.deepEqual(await page.evaluate(()=>({project:localStorage.getItem('patterncanvas-iphone-v1'),scrollY,work:document.getElementById('work').scrollTop})),stateBeforeTurn);
  await page.locator('#chart-mirror').click();
  assert.equal(await c.evaluate(c=>c.toDataURL()),imageBeforeTurn,'Turning back restores the exact chart without scroll drift');
  ...
  console.log('PASS zoomed/panned turning retains visible stitches, scroll and exact roundtrip; accessible gear opens settings');
