...
try{
 ...
 await page.locator('#memo-open').click();await page.locator('#memo').fill('糸を替える');await page.locator('#memo-save').click();await page.reload();await page.locator('#chart').waitFor({state:'visible'});assert.equal((await state()).notes[2],'糸を替える');console.log('PASS WebKit: row gutter, complete/back, undo/redo, note and reload');
 ...
 assert.equal(await page.evaluate(async()=>!!(await caches.match(location.href))),true);await page.reload();await page.locator('#chart').waitFor({state:'visible'});assert.equal((await state()).currentRow,4);assert.ok(await page.locator('#chart').isVisible());console.log('PASS WebKit: repeated row jump, phone widths, cached app shell and reload');
 ...
 await page.locator('[data-view=settings]').click();await page.locator('#import').setInputFiles({name:'sample.json',mimeType:'application/json',buffer:Buffer.from(JSON.stringify(sample()))});await page.setViewportSize({width:390,height:844});await page.reload();await page.locator('#chart').waitFor({state:'visible'});await page.waitForFunction(()=>getComputedStyle(document.querySelector('[data-tool=pen]')).backgroundColor==='rgb(82, 121, 107)');await page.screenshot({path:'docs/iphone/app-screen.png'});
