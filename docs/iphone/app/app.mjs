...
import {draw,chartColumn,CHART_FOOTER,CHART_GUTTER} from './draw.mjs';
...
const chartView={cell:24,x:CHART_GUTTER,y:0,mirrored:false},previewView={cell:12,x:0,y:0};
...
try{chartView.mirrored=localStorage.getItem('patterncanvas-chart-mirrored')==='true';}catch{}
...
function gestures(canvas,view,isPreview){const pointers=new Map();let before=null,last=null,multi=false,start=null,moved=false;
 ...
 const cell=pos=>{if(!isPreview&&pos.y>=canvas.clientHeight-CHART_FOOTER)return null;const x=Math.floor((pos.x-view.x)/view.cell),r=M.totalRows(p)-1-Math.floor((pos.y-view.y)/view.cell);return x>=0&&r>=0&&x<M.totalColumns(p)&&r<M.totalRows(p)?{x:chartColumn(x,M.totalColumns(p),!isPreview&&view.mirrored)%p.columns,r:r%p.rows,displayRow:r+1}:null;};
...
$('chart-mirror').onclick=()=>{chartView.mirrored=!chartView.mirrored;try{localStorage.setItem('patterncanvas-chart-mirrored',String(chartView.mirrored));}catch{message('表示方向を保存できませんでした。');}controls();paintCanvases();};
