...
function gestures(canvas,view,isPreview){const pointers=new Map();let before=null,last=null,multi=false,start=null,moved=false;
 ...
 const cell=pos=>{if(!isPreview&&(pos.y<CHART_HEADER||pos.y>=canvas.clientHeight-CHART_FOOTER))return null;const x=Math.floor((pos.x-view.x)/view.cell),r=M.totalRows(p)-1-Math.floor((pos.y-view.y)/view.cell);return x>=0&&r>=0&&x<M.totalColumns(p)&&r<M.totalRows(p)?{x:x%p.columns,r:r%p.rows,displayRow:r+1}:null;};
