...
import {draw,CHART_FOOTER,CHART_GUTTER} from './draw.mjs';
...
const chartView={cell:24,x:CHART_GUTTER,y:0},previewView={cell:12,x:0,y:0};
let locked=false,readability={color:'#8d1735',width:4};
try{locked=localStorage.getItem('patterncanvas-chart-locked')==='true';const saved=JSON.parse(localStorage.getItem('patterncanvas-readability')||'{}');if(/^#[0-9a-f]{6}$/i.test(saved.color))readability.color=saved.color;if([2,4,6].includes(saved.width))readability.width=saved.width;}catch{}
...
document.querySelectorAll('[data-view]').forEach(b=>b.onclick=()=>setView(b.dataset.view));document.querySelectorAll('[data-tool]').forEach(b=>b.onclick=()=>{if(!editable())return;tool=b.dataset.tool;clipboard=null;controls();});
...
$('complete').onclick=()=>{change(()=>M.complete(p));focus();};$('previous').onclick=()=>{change(()=>M.previous(p));focus();};$('choose-row').onclick=()=>{$('row-input').value=M.rowNumber(p,p.currentRow);$('row-input').max=M.totalRows(p);$('row-dialog').showModal();};
...
function replace(next){if(!editable())return;change(()=>p=next);selection=null;clipboard=null;tool='pen';yarn=p.yarns[0].id;setView('both');render();}
...
function structureOptions(){
 ...
 return {kind,options:{action,count:number('structure-count',1,100),fromRow:number('structure-row',1,M.totalRows(p)+(kind==='rows'&&action==='add'?1:0)),position:kind==='rows'?number('structure-row',1,M.totalRows(p)+(action==='add'?1:0)):$('structure-edge').value==='position'?number('structure-position',1,2001):1,edge:$('structure-edge').value,color:yarn}};
}
...
$('chart-lock').onclick=()=>{locked=!locked;selection=null;clipboard=null;try{localStorage.setItem('patterncanvas-chart-locked',String(locked));}catch{message('ロック状態を保存できませんでした。');}render();settings();};
