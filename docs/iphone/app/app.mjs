...
import {draw,CHART_FOOTER} from './draw.mjs';
...
let locked=false;try{locked=await Platform.readLock();}catch{}
...
function focus(){chartView.y=($('chart').clientHeight-CHART_FOOTER)/2-(M.totalRows(p)-p.currentRow+.5)*chartView.cell;paintCanvases();}
...
$('chart-lock').onclick=()=>{locked=!locked;selection=null;clipboard=null;try{Promise.resolve(Platform.writeLock(locked)).catch(()=>message('ロック状態を保存できませんでした。'));}catch{message('ロック状態を保存できませんでした。');}render();};
$('complete').onclick=()=>{change(()=>M.complete(p));Platform.rowCompleted();focus();};$('previous').onclick=()=>{change(()=>M.previous(p));focus();};$('choose-row').onclick=()=>{$('row-input').value=p.currentRow;$('row-input').max=M.totalRows(p);$('row-dialog').showModal();};
...
function gestures(canvas,view,isPreview){const pointers=new Map();let before=null,last=null,multi=false,start=null,moved=false;
 ...
 const cell=pos=>{if(!isPreview&&pos.y>=canvas.clientHeight-CHART_FOOTER)return null;const x=Math.floor((pos.x-view.x)/view.cell),r=M.totalRows(p)-1-Math.floor((pos.y-view.y)/view.cell);return x>=0&&r>=0&&x<M.totalColumns(p)&&r<M.totalRows(p)?{x:x%p.columns,r:r%p.rows,displayRow:r+1}:null;};
 function apply(pos){if(locked)return;const at=cell(pos);if(!at||pos.x<42)return;if(tool==='pen'||tool==='eraser'){if(last){const count=Math.max(Math.abs(at.x-last.x),Math.abs(at.r-last.r));if(count<Math.max(p.rows,p.columns)/2)for(let i=1;i<=count;i++)M.paint(p,Math.round(last.x+(at.x-last.x)*i/count),Math.round(last.r+(at.r-last.r)*i/count),tool==='pen'?yarn:p.yarns[0].id);}M.paint(p,at.x,at.r,tool==='pen'?yarn:p.yarns[0].id);last=at;}
...
$('add-color').onclick=()=>{if(!editable())return;if(p.yarns.length>=32)return message('毛糸は32色まで追加できます。');change(()=>{const id=Math.max(...p.yarns.map(y=>y.id))+1;p.yarns.push({id,name:$('new-color-name').value.trim()||`毛糸 ${id+1}`,color:$('new-color').value});yarn=id;});settings();};
function structureOptions(){
 const [action,kind]=$('structure-action').value.split('-');
 return {kind,options:{action,count:number('structure-count',1,100),fromRow:number('structure-row',1,M.totalRows(p)+(kind==='rows'&&action==='add'?1:0)),position:kind==='rows'?number('structure-row',1,M.totalRows(p)+(action==='add'?1:0)):$('structure-edge').value==='position'?number('structure-position',1,1001):1,edge:$('structure-edge').value,color:yarn}};
}
function structurePreview(){
 const [action,kind]=$('structure-action').value.split('-');
 $('structure-edge-fields').hidden=kind==='rows';$('structure-position-label').hidden=$('structure-edge').value!=='position';
 $('structure-position-text').textContent=action==='add'?'この目番号の前に挿入（左から）':'削除を始める目番号（左から）';
 $('structure-row-label').textContent=kind==='stitches'?'変更を始める段（この段から上）':action==='add'?'この段の前に挿入（最後の段＋1で上端に追加）':'削除を始める段';
 ...
 try{
  const {options}=structureOptions(),next=M.clone(p);if(kind==='stitches')M.editStitches(next,options);else M.editRows(next,options);
  const row=options.fromRow;
  let text=kind==='stitches'?`${row}〜${M.totalRows(p)}段目を変更。${row}段目は ${M.stitchCount(p,row)}目 → ${M.stitchCount(next,row)}目。`:`${options.position}段目から${options.count}段${action==='add'?'挿入':'削除'}。全${M.totalRows(p)}段 → ${next.rows}段。`;
...
$('stitch-edit').onclick=()=>{if(!editable())return;$('structure-row').value=p.currentRow;$('structure-count').value=1;structurePreview();$('structure-dialog').showModal();};
$('structure-close').onclick=()=>$('structure-dialog').close();
for(const id of ['structure-action','structure-row','structure-edge','structure-position','structure-count'])$(id).oninput=structurePreview;
$('structure-apply').onclick=()=>{if(!editable())return;try{const {kind,options}=structureOptions(),next=M.clone(p);if(kind==='stitches')M.editStitches(next,options);else M.editRows(next,options);change(()=>p=next);selection=null;clipboard=null;tool='pen';$('structure-dialog').close();render();fitPreview();focus();message('目・段の変更を保存しました。「元に戻す」で取り消せます。');}catch(e){$('structure-summary').textContent=e.message;}};
function replace(next){if(!editable())return;canSave=true;change(()=>p=next);selection=null;clipboard=null;tool='pen';yarn=p.yarns[0].id;setView('chart');render();}
$('new-project').onclick=()=>{if(!editable())return;try{const w=number('new-width',1,100),h=number('new-height',1,100);if(confirm('現在の作品を新しい白紙に置き換えます。必要なら先に作品を書き出してください。'))replace(M.blank(w,h));}catch(e){message(e.message);}};
...
$('import').onchange=async e=>{if(!editable()){e.target.value='';return;}const file=e.target.files[0];try{if(!file)return;if(file.size>2000000)throw Error('作品ファイルは2MB以下にしてください');const next=M.validate(JSON.parse(await file.text()));if(confirm('このファイルで現在の作品を置き換えますか？'))replace(next);}catch(err){message('読み込みできません：'+err.message);}finally{e.target.value='';}};
...
$('image-file').onchange=async e=>{if(!editable()){e.target.value='';return;}let url;try{const file=e.target.files[0];if(!file)return;if(file.size>20000000)throw Error('画像は20MB以下にしてください');number('new-width',1,100);number('new-height',1,100);url=URL.createObjectURL(file);sourceImage=new Image();sourceImage.src=url;await sourceImage.decode();$('crop-zoom').value=1;$('crop-x').value=.5;$('crop-y').value=.5;showCrop();$('image-dialog').showModal();}catch(err){message('画像を読み込めません：'+err.message);}finally{if(url)URL.revokeObjectURL(url);e.target.value='';}};
