...
import {draw,chartColumn,turnChart,CHART_FOOTER,CHART_GUTTER} from './draw.mjs';
...
$('chart-mirror').onclick=()=>{turnChart(chartView,M.totalColumns(p),$('chart').getBoundingClientRect().width);try{localStorage.setItem('patterncanvas-chart-mirrored',String(chartView.mirrored));}catch{message('表示方向を保存できませんでした。');}controls();paintCanvases();};
