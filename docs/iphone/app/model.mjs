...
export function validate(p) {
  ...
  if(!p||!['patterncanvas-web-1','patterncanvas-web-2'].includes(p.format)||!integer(p.columns,1,2000)||!integer(p.rows,1,2000)||!integer(p.horizontalRepeats,1,10)||!integer(p.verticalRepeats,1,10)||totalRows(p)>2000||totalColumns(p)>2000)throw Error('対応していない作品ファイルです');
...
function reversedCoordinates(p){const n=totalRows(p),q=clone(p);commitShape(q,expandedCells(p).reverse());q.currentRow=n+1-p.currentRow;q.completedRows=p.completedRows.map(r=>n+1-r);q.notes=Object.fromEntries(Object.entries(p.notes).map(([r,note])=>[n+1-Number(r),note]));return q;}
